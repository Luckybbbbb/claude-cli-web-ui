...
export function createClaudeStreamHandler(onEvent: EventSink) {
  ...
  function handleObject(obj: unknown) {
    ...
    if (obj.type === 'system' && obj.subtype === 'status') {
      onEvent({ type: 'status', label: obj.status ?? 'working' } as AgentEvent);
      return;
    }
    ...
    if (obj.type === 'assistant' && isRecord(obj.message) && Array.isArray(obj.message.content)) {
      ...
      for (const block of obj.message.content) {
        ...
        if (block.type === 'tool_use') {
          ...
          onEvent({ type: 'text_delta', delta: block.text } as AgentEvent);
        } else if (
          !alreadyStreamed &&
          block.type === 'thinking' &&
          ...
          onEvent({ type: 'thinking_delta', delta: block.thinking } as AgentEvent);
        }
      }
      ...
      if (stopReason) {
        onEvent({ type: 'turn_end', stopReason } as AgentEvent);
      }
      return;
    }
  ...
  function handleStreamEvent(ev: Record<string, unknown>) {
    if (ev.type === 'message_start') {
      ...
      if (typeof ev.ttft_ms === 'number') {
        onEvent({ type: 'status', label: 'streaming', ttftMs: ev.ttft_ms } as AgentEvent);
      }
      return;
    }
    ...
    if (ev.type === 'content_block_start' && isRecord(ev.content_block)) {
      ...
      if (block.type === 'thinking') {
        onEvent({ type: 'thinking_delta', delta: '' } as AgentEvent);
      }
      return;
    }
    ...
    if (ev.type === 'content_block_delta' && isRecord(ev.delta)) {
      ...
      if (delta.type === 'text_delta' && typeof delta.text === 'string') {
        ...
        onEvent({ type: 'text_delta', delta: delta.text } as AgentEvent);
        return;
      }
      if (delta.type === 'thinking_delta' && typeof delta.thinking === 'string') {
        if (currentMessageId) textStreamed.add(currentMessageId);
        onEvent({ type: 'thinking_delta', delta: delta.thinking } as AgentEvent);
        return;
      }
