...
### 核心概念
- **stream-json 协议**: Claude CLI 的 JSONL 输出格式，包含 system/stream_event/assistant/user/result 五种顶层消息类型
- **SSE 广播**: 服务端使用 Server-Sent Events 将解析后的事件推送给客户端
- **双版本兼容**: 同时处理有/无 --include-partial-messages 的输出格式

### 关键数据结构
- `RunState`: 运行状态（id, status, events[], child process, SSE clients Set）
...
### 核心流程
- **服务端**: spawn Claude CLI -> stdout JSONL -> claude-stream.ts 解析 -> AgentEvent -> runs.ts 存储 + SSE 广播
- **客户端**: POST /api/chat 获取 runId -> fetch /api/runs/{id}/events -> ReadableStream 手动解析 SSE -> UI 更新

### 与其他系统的交互
- **Claude CLI**: child_process.spawn，--print 模式，shell: true（Windows 兼容）
- **UI 组件**: AgentEvent[] -> AssistantMessage -> ThinkingBlock/ToolCard/Markdown
<!-- OVERVIEW_END -->
